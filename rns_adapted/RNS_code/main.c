...
void print(double r_ratio,
           ...
           double Omega, double Omega_K, double J, FILE* outfile
          )
{
	double I_45;
...
void printpoly(double n_P, double r_ratio,
               double e_center, double Mass, double Mass_0, double R_e,
               double Omega, double Omega_K, double J, FILE* outfile
              )
{
	fprintf(outfile, "%g\t%g\t%g\t%.10f\t%.10f\t%g\t%g\t%g\t%.9e\n",
	    n_P,
	    r_ratio,
	    e_center,
	    Mass,
         ...
         char **argv)                 /* Command line arguments */
{
	...
	double cf = 0.5, /* convergence factor */
	       accuracy;
	...
	char out_filename[80] = "NONE"; /* Output file */
	...
	for(i = 1; i < argc; i++)
	{
		if(argv[i][0] == '-')
		{
			...
			case 'd':
				/* AXES RATIO STEP */
				sscanf(argv[i + 1], "%lf", &dr);
				break;

			case 'o':
				/* CHOOSE THE NAME OF THE OUTPUT FILE */
				sscanf(argv[i + 1], "%s", out_filename);
                out_to_file = true;
				break;
			...
			case 'h':
				...
				fprintf(stderr, "  -e central energy density (in gr/cm^3 if EOS is \"tab\")\n");
                fprintf(stderr, "  -d axis ratio step\n");
    ...
    if(!out_to_file) {
        ...
        printf("N\tGamma\tratio\te_15\tM\tM_0\tr_star\tOmega\tOmega_K\tJ\tJ/M^2\n");
    }
	...
	if (strcmp(eos_type, "tab") == 0)
		print(r_ratio, e_center, Mass, Mass_0, R_e, Omega, Omega_K, J, outfile);
	else
		printpoly(n_P, r_ratio, e_center, Mass, Mass_0, R_e, Omega, Omega_K, J, outfile);
